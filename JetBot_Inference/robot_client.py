...
class Bot:
    ...
    def calibrate_image(self, image_filename):
        ...
        dst = cv2.undistort(image, self.mtx, self.dist, None, newcameramtx)  # Apply undistortion
...
def keyboard_killswitch(q: queue.Queue, done: threading.Event):
...
def main():
    ...
    look_thread = threading.Thread(target=keyboard_killswitch, args=(q, done))  # Start thread to monitor user input
